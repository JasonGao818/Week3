...
protected:	
	...
	ofVec3f velocity;
    ofPath arrow;
    ofVec3f separationBetweenTypes(std::vector<Boid *> &boids1, std::vector<Boid *> &boids2);
...
public:	
    ...
    void setAttractionWeight(float f); // new method
    void setRepulsionWeight(float f); // new method
	
	//void update(std::vector<Boid *> &otherBoids, ofVec3f &min, ofVec3f &max);
    void update(std::vector<Boid *> &otherBoids, std::vector<Boid *> &sameTypeBoids, ofVec3f &min, ofVec3f &max); // modified method
